...
static void ComputeChildLayout(ksd::ChildState& c)
{
    ...
    const ImVec2 minSize = ImVec2(
        (boundsSize.x - 2.f) * 0.224f,
        (boundsSize.y - 2.f) * 0.134f);
    const ImVec2 step = ImVec2(
        (boundsSize.x - minSize.x - 2.f) / 9.f,
        (boundsSize.y - minSize.y - 2.f) / 10.f);

    c.pos  = boundsMin + ImVec2(c.xPos * step.x, c.yPos * step.y);
    c.size = ImVec2(minSize.x + c.xSize * step.x, minSize.y + c.ySize * step.y);
}
