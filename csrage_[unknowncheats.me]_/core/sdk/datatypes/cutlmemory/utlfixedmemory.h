...
public:
	...
	void Grow(const int nCount = 1)
	{
		...
		BlockHeader_t* pNewBlockHeader = static_cast<BlockHeader_t*>(malloc(sizeof(BlockHeader_t) + nBlockSize * sizeof(T)));
		debug(pNewBlockHeader != nullptr); // container overflow
