...
#include "../../sdk/datatypes/cutlhash/utlthash.h"
...
public:
	...
	{
		return utils::CallVFunc<CSchemaType*, 4U>(this, szName, pSchema);
	}

	CSchemaType* FindTypeDeclaredClass(const char* szName)
	{
		return utils::CallVFunc<CSchemaType*, 5U>(this, szName);
	}

	CSchemaType* FindTypeDeclaredEnum(const char* szName)
	{
		return utils::CallVFunc<CSchemaType*, 6U>(this, szName);
	}

	CSchemaClassBinding* FindRawClassBinding(const char* szName)
	{
		return utils::CallVFunc<CSchemaClassBinding*, 7U>(this, szName);
	}

	void* pVtable; // 0x0000
	char szName[256U]; // 0x0008
	pad(SCHEMASYSTEMTYPESCOPE_OFF1); // 0x0108
	CUtlTSHash<CSchemaClassBinding*, 256, unsigned int> hashClasses; // 0x0588
	pad(SCHEMASYSTEMTYPESCOPE_OFF2); // 0x05C8
...
private:
	pad(SCHEMASYSTEM_TYPE_SCOPES_OFFSET); // 0x0000
public:
