...
d3d11_manager::d3d11_manager(ID3D11Device* dev, ID3D11DeviceContext* ctx) : _device_ptr(dev), _ctx(ctx) {
	init();
	_tex_dict = new dx11::tex_dict_dx11;
}
...
{
	auto texture = _tex_dict->create_texture(width, height);

	return reinterpret_cast<tex_id>(_tex_dict->create_texture(width, height));
}
ID3D11ShaderResourceView* d3d11_manager::get_texture(const uint32_t width, const uint32_t height)
{
	auto texture = _tex_dict->create_texture(width, height)->texture();

	return reinterpret_cast<ID3D11ShaderResourceView*>(texture);
}
...
#include "..\..\utils.h"
bool d3d11_manager::set_texture_rgba(ID3D11ShaderResourceView* id, const uint8_t* rgba,
	const uint32_t width, const uint32_t height)
{
	debug(id != nullptr);
