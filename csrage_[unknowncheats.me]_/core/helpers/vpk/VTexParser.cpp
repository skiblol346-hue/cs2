...
auto get_buffer(const std::string_view path, int32_t& file_size) {
...
VTexParser::ImageData VTexParser::Load(std::string_view filename) {
	...
	/*auto handle = sdk::m_file_system->Open(filename.data(), xorstr_("r"), xorstr_("GAME"));
	auto size = sdk::m_file_system->Size(handle);
	std::vector<char> image(size);
	sdk::m_file_system->Read(image.data(), size, handle);
	sdk::m_file_system->Close(handle);
	*/
	int32_t size = 0;
	//auto handle = get_buffer(filename.data(), size);
	auto image = get_buffer(filename.data(), size);
	...
	for (int i = 0; i < size; i++) {
		if (*(uint32_t*)(image.get() + i) == *(uint32_t*)xorstr_("DATA")) {
			hdrPos = *(uint32_t*)(image.get() + i + 4) + i + 4;
			break;
		}
	}

	memstream s((char*)image.get());
	s.pos = hdrPos;
