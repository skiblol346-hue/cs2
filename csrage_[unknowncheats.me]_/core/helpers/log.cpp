// used: [c_run_time] time_t, time, localtime_s
#include <ctime>
...
bool GetWorkingPath2(wchar_t* wszDestination)
{
	bool bSuccess = false;
...
bool L::OpenFile(const wchar_t* wszFileName)
{
	...
	if (!GetWorkingPath2(wszFilePath))
		return false;

	c_run_time::StringCat(wszFilePath, wszFileName);
...
L::Stream_t& L::Stream_t::operator()(const ELogLevel nLevel, const char* szFileBlock)
{
	...
	const std::size_t nTimeSize = c_run_time::TimeToString(szTimeBuffer, sizeof(szTimeBuffer), "\n[%d-%m-%Y %T] ", &timePoint) - bFirstPrint;
	...
	if (szFileBlock != nullptr)
	{
		...
		::WriteConsoleA(hConsoleStream, szFileBlock, c_run_time::StringLength(szFileBlock), nullptr, nullptr);
	}

	if (szTypeBlock != nullptr)
	{
		::SetConsoleTextAttribute(hConsoleStream, static_cast<WORD>(nTypeColorFlags));
		::WriteConsoleA(hConsoleStream, szTypeBlock, c_run_time::StringLength(szTypeBlock), nullptr, nullptr);
	}
	...
	if (szFileBlock != nullptr)
		szCurrentBlock = c_run_time::StringCat(szCurrentBlock, szFileBlock);

	if (szTypeBlock != nullptr)
		szCurrentBlock = c_run_time::StringCat(szCurrentBlock, szTypeBlock);
...
{
#if defined(CS_LOG_CONSOLE) || defined(CS_LOG_FILE)
	WriteMessage(szMessage, c_run_time::StringLength(szMessage));
#endif
	return *this;
}
...
L::Stream_t& L::Stream_t::operator<<(const wchar_t* wszMessage)
{
	 ...
	 */
	const std::size_t nMessageLength = c_run_time::StringLengthMultiByte(wszMessage);
	char* szMessage = static_cast<char*>(MEM_STACKALLOC(nMessageLength + 1U));
	c_run_time::StringUnicodeToMultiByte(szMessage, nMessageLength + 1U, wszMessage);
...
L::Stream_t& L::Stream_t::operator<<(const bool bValue)
{
	...
	const std::size_t nBooleanLength = c_run_time::StringLength(szBoolean);
