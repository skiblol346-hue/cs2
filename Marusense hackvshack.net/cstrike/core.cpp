...
bool CORE::GetWorkingPath(wchar_t* wszDestination)
{
	...
	{
		CRT::StringCat(CRT::StringCopy(wszDestination, wszPathToDocuments), CS_XOR(L"\\.marusense\\"));
		bSuccess = true;
...
static bool Setup(HMODULE hModule)
{
	...
	if (!L::AttachConsole(CS_XOR(L"marusense")))
	{
...
#endif
#ifdef CS_LOG_FILE
	if (!L::OpenFile(CS_XOR(L"marusense.log")))
	{
	...
	L_PRINT(LOG_NONE) << L::SetColor(LOG_COLOR_FORE_CYAN | LOG_COLOR_FORE_INTENSITY) << CS_XOR("��� �� � ���� ����� initialization completed, version: ") << CS_STRINGIFY(CS_VERSION);
	return true;
}
