...
bool F::VISUALS::CHAMS::Initialize()
{
	...
	.pMaterial = CreateMaterial(
		...
		szVMatBufferWhiteVisible
	),
	.pMaterialInvisible = CreateMaterial(
		"materials/dev/primary_white_invisible.vmat",
		szVMatBufferWhiteInvisible
	)
	};

	arrMaterials[VISUAL_MATERIAL_ILLUMINATE] = CustomMaterial_t{
		.pMaterial = CreateMaterial(
			"materials/dev/illuminate.vmat",
			szVMatBufferIlluminateVisible
		),
		.pMaterialInvisible = CreateMaterial(
			"materials/dev/illuminate_invisible.vmat",
			szVMatBufferIlluminateInvisible
		)
	};
	...
	arrMaterials[VISUAL_MATERIAL_LATEX] = CustomMaterial_t{
		.pMaterial = CreateMaterial("materials/dev/latex_visible.vmat", szVMatBufferLatexVisible),
		.pMaterialInvisible = CreateMaterial("materials/dev/latex_invisible.vmat", szVMatBufferLatexInvisible)
	};


	arrMaterials[VISUAL_MATERIAL_GHOST] =
	{
		.pMaterial = CreateMaterial(
			"materials/dev/ghost_visible.vmat",
			szVMatBufferGlowVisible
		),
		.pMaterialInvisible = CreateMaterial(
			"materials/dev/ghost_invisible.vmat",
		szVMatBufferGlowInisible
		)
	};
...
bool F::VISUALS::CHAMS::OnDrawObject2(
	...
	void* pUnk2)
{
	...
	if (CRT::StringCompare(pClassInfo->Name, CS_XOR("C_CSPlayerPawn")) != 0)
		return false;
...
bool F::VISUALS::CHAMS::OnDrawObject(
	void* pAnimatableSceneObjectDesc,
	void* pDx11,
	CMeshData* arrMeshDraw,
	int nDataCount,
...
{
	CHAMS::OnDrawObject2(pAnimatableSceneObjectDesc, pDx11, arrMeshDraw, nDataCount, pSceneView, pSceneLayer, pUnk, pUnk2);
	if (!bInitialized)
		return false;
	...
	auto pEntity = I::GameResourceService
		->pGameEntitySystem
		->Get<C_BaseEntity>(hOwner);
	...
	FNV1A_t classHash = FNV1A::Hash(pClassInfo->Name);

	if (C_GET(bool, Vars.bVisualChamsWeapon) && classHash == FNV1A::HashConst("C_CS2HudModelWeapon"))
	{
		return OverrideMaterial(pAnimatableSceneObjectDesc, pDx11, arrMeshDraw, nDataCount,
			pSceneView, pSceneLayer, pUnk, pUnk2);
	}

	if (C_GET(bool, Vars.bVisualChamsArms) && classHash == FNV1A::HashConst("C_CSGOViewModel"))
	{
		return OverrideMaterial(pAnimatableSceneObjectDesc, pDx11, arrMeshDraw, nDataCount,
			pSceneView, pSceneLayer, pUnk, pUnk2);
	}
