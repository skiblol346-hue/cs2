...
#include "sdk.h"
#include "../sdk/interfaces/scenesystem.h"
...
bool H::Setup()
{
	...
	if (!hkGetMatrixForView.Create(MEM::FindPattern(CLIENT_DLL, CS_XOR("48 8B C4 48 89 68 08")), reinterpret_cast<void*>(&GetMatrixForView)))
		return false;
	...
	if (!hkCreateMove.Create(MEM::FindPattern(CLIENT_DLL, CS_XOR("48 8B C4 4C 89 40 ? 48 89 48 ? 55 53 41 54")), reinterpret_cast<void*>(&CreateMove)))
		return false;
	...
	if (!hkLevelInit.Create(MEM::FindPattern(CLIENT_DLL, CS_XOR(" 40 55 56 41 56 48 8D 6C 24 ? 48 81 EC ? ? ? ? 48 8B 0D")), reinterpret_cast<void*>(&LevelInit)))
		return false;
	...
	if (!hkOverrideView.Create(MEM::FindPattern(CLIENT_DLL, CS_XOR("40 57 48 83 EC ? 48 8B FA E8 ? ? ? ? BA")), reinterpret_cast<void*>(&OverrideView)))
		return false;
	L_PRINT(LOG_INFO) << CS_XOR("\"OverrideView\" hook has been created");

	// Credit: https://www.unknowncheats.me/forum/4253223-post6185.html
	F::LEGITBOT::LEGIT::InitSpreadFunctions();
	...
	if (!DrawViewModel.Create(MEM::FindPattern(CLIENT_DLL, CS_XOR("40 55 53 56 41 56 41 57 48 8B EC 48 83 EC 20 4D 8B F8 4C 8B F2 48 8B")), reinterpret_cast<void*>(&hkDrawViewModel)))
		return false;
	L_PRINT(LOG_INFO) << CS_XOR("\"DrawViewModel\" hook has been created");


	if (!GetFieldOfView.Create(MEM::FindPattern(CLIENT_DLL, CS_XOR("40 53 48 83 EC ? 48 8B D9 E8 ? ? ? ? 48 85 C0 74 ? 48 8B C8 48 83 C4")), reinterpret_cast<void*>(&hkGetFieldOfView)))
		return false;
	L_PRINT(LOG_INFO) << CS_XOR("\"GetRenderFov\" hook has been created");

	if (!hkDrawObject.Create(MEM::FindPattern(SCENESYSTEM_DLL, CS_XOR("48 8B C4 53 57 41 54")), reinterpret_cast<void*>(&DrawObject)))
		return false;
	L_PRINT(LOG_INFO) << CS_XOR("\"DrawObject\" hook has been created");

	if (!hkSetInfo.Create(MEM::FindPattern(ENGINE2_DLL, CS_XOR("40 55 41 57 48 8D 6C 24 ? 48 81 EC ? ? ? ? 45 33 FF")), reinterpret_cast<void*>(&SetInfo)))
		return false;
	L_PRINT(LOG_INFO) << CS_XOR("\"SetInfo\" hook has been created");

	L_PRINT(LOG_INFO) << CS_XOR("\"DrawSmokeArray\" hook has been created");
	if (!DrawSmokeArray.Create(MEM::FindPattern(CLIENT_DLL, CS_XOR("48 89 5C 24 ? 48 89 6C 24 ? 48 89 74 24 ? 57 41 56 41 57 48 83 EC ? 48 8B 9C 24 ? ? ? ? 4D 8B F8")), reinterpret_cast<void*>(&hkDrawSmokeArray)))
		return false;



	L_PRINT(LOG_INFO) << CS_XOR("\"GlowManagerApplyGlow\" hook has been created");
	return true;

}


void __fastcall H::OverrideView(void* pClientModeCSNormal, CViewSetup* pSetup) {
	const auto oOverrideView = hkOverrideView.GetOriginal();
	oOverrideView(pClientModeCSNormal, pSetup);
}

float H::hkGetFieldOfView(void* a1) {
	SDK::LocalController = CCSPlayerController::GetLocalPlayerController();
	if (SDK::LocalController == nullptr)
		return H::GetFieldOfView.GetOriginal()(a1);
...
{
	hkPresent.Remove();
	hkResizeBuffers.Remove();
	hkCreateSwapChain.Remove();
	hkWndProc.Remove();

	hkCreateMove.Remove();
	hkMouseInputEnabled.Remove();
	hkIsRelativeMouseMode.Remove();
	hkFrameStageNotify.Remove();
	hkLevelInit.Remove();
	hkLevelShutdown.Remove();

	GetFieldOfView.Remove();
	DrawViewModel.Remove();
	hkOverrideView.Remove();
	hkSetInfo.Remove();
...
void* __fastcall H::hkDrawSmokeArray(void* rcx, void* pSomePointer1, void* pSomePointer2, void* pSomePointer3, void* pSomePointer4, void* pSomePointer5) {
	if (C_GET(bool, Vars.bAntiSmoke)) return nullptr;
...
HRESULT CS_FASTCALL H::ResizeBuffers(
	IDXGISwapChain* pSwapChain,
	std::uint32_t nBufferCount,
	std::uint32_t nWidth,
	...
	std::uint32_t nFlags)
{
	...
	if (I::RenderTargetView)
	{
		...
		I::RenderTargetView = nullptr;
	}

	HRESULT hResult = oResizeBuffer(pSwapChain, nBufferCount, nWidth, nHeight, newFormat, nFlags);
...
void* __fastcall H::hkDrawViewModel(float* a1, float* offsets, float* fov) {
	...
	if (C_GET(bool, Vars.viewmodell)) {
		offsets[0] = (C_GET(float, Vars.viewx));
		offsets[1] = (C_GET(float, Vars.viewy));
		offsets[2] = (C_GET(float, Vars.viewz));
		*fov = (C_GET(float, Vars.viewfov));
	}
	return pRet;
}
...
bool CS_FASTCALL H::CreateMove(CCSGOInput* pInput, int nSlot, CUserCmd* cmd)
{
	...
	SDK::LocalPawn =
		I::GameResourceService->pGameEntitySystem
		->Get<C_CSPlayerPawn>(SDK::LocalController->GetPawnHandle());

	if (!SDK::LocalPawn)
		return bResult;

	auto sceneNode = SDK::LocalPawn->GetGameSceneNode();
	if (!sceneNode)
		return bResult;

	F::OnCreateMove(cmd, pBaseCmd, SDK::LocalController, SDK::LocalPawn);

	F::LEGITBOT::LEGIT::OnMove(cmd, pBaseCmd, SDK::LocalController, SDK::LocalPawn);
...
bool CS_FASTCALL H::MouseInputEnabled(void* pThisptr)
{
	...
	return gui::open ? false : oMouseInputEnabled(pThisptr);
}
...
char CS_FASTCALL H::SetInfo(void* rcx, void* a2)
{
	const auto oSetInfo = hkSetInfo.GetOriginal();
	if (!rcx || !a2)
	...
	if (nameVar && nameVar->value.sz)
	{
		nameVar->nFlags &= ~FCVAR_PROTECTED;
		nameVar->nFlags |= FCVAR_USERINFO;
	}

	if (reinterpret_cast<uintptr_t>(a2) < 0x1000)
		return oSetInfo(rcx, a2);

	// offsets may need updating after CS2 patches
	uintptr_t argList = *reinterpret_cast<uintptr_t*>(reinterpret_cast<uintptr_t>(a2) + 0x440);
	if (argList < 0x1000)
		return oSetInfo(rcx, a2);

	const char* key = *reinterpret_cast<const char**>(argList + 0x8);
	const char** valuePtr = reinterpret_cast<const char**>(argList + 0x10);

	if (!key || !valuePtr || !*valuePtr)
		return oSetInfo(rcx, a2);

	static std::string originalName = "player";
	if (nameVar && nameVar->value.sz)
	{
		std::string current = nameVar->value.sz;
		if (current.find("maru") == std::string::npos && !current.empty())
			originalName = current;
	}

	if (_stricmp(key, "name") == 0)
	{
		static const char* base = "maru";
		constexpr int baseLen = 4;
		constexpr int totalSteps = baseLen * 2;

		auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(
			std::chrono::steady_clock::now().time_since_epoch()).count();
		int step = static_cast<int>((ms / 400) % totalSteps);

		char animBuf[8];
		if (step < baseLen)
		{
			memcpy(animBuf, base, step + 1);
			animBuf[step + 1] = '\0';
		}
		else
		{
			int keep = totalSteps - step;
			memcpy(animBuf, base, keep);
			animBuf[keep] = '\0';
		}
		...
		static std::string finalName;
		finalName = std::string(animBuf) + " " + originalName;
		*valuePtr = finalName.c_str();
	}
