...
namespace H
{
	...
	float hkGetFieldOfView(void* a1);
	void __fastcall OverrideView(void* pClientModeCSNormal, CViewSetup* pSetup);
	char CS_FASTCALL SetInfo(void* rcx, void* a2);
	...
	inline CBaseHookObject<decltype(&hkDrawViewModel)> DrawViewModel = {};
	...
	inline CBaseHookObject<decltype(&hkDrawSmokeArray)>  DrawSmokeArray = {};
	inline CBaseHookObject<decltype(&hkGetFieldOfView)> GetFieldOfView = {};
	inline CBaseHookObject<decltype(&OverrideView)> hkOverrideView = {};
	inline CBaseHookObject<decltype(&SetInfo)> hkSetInfo = {};
}
