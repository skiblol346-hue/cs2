...
public:
	...
	void Grow( const int nCount = 1 )
	{
		...
		BlockHeader_t* pNewBlockHeader = static_cast< BlockHeader_t* >( GetInterfaceManager( )->GetMemAlloc( )->Alloc( sizeof( BlockHeader_t ) + nBlockSize * sizeof( T ) ) );
	...
	void Purge( )
	{
		...
		for ( BlockHeader_t* pBlockHeader = pBlocks; pBlockHeader != nullptr;)
		{
			...
			pBlockHeader = pBlockHeader->pNext;
			GetInterfaceManager( )->GetMemAlloc( )->Free( pFree );
		}
