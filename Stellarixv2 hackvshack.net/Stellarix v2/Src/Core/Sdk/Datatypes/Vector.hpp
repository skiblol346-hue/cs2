...
#pragma region vector_array_operators
	...
	float dot( Vector_t v, bool additional )
	{
		if ( additional )
...
#pragma endregion
	...
	[[nodiscard]] constexpr float DotProduct( const Vector_t& vecDot, const bool bAdditional = false ) const
	{
		if ( bAdditional )
...
struct alignas( 16 ) VectorAligned_t : Vector_t
{
	...
	{
		Vector_t::operator=( vecBase );
		w = 0.0f;
	}

	VectorAligned_t& operator=( const Vector_t& vecBase )
	{
		Vector_t::operator=( vecBase );
		w = 0.0f;
		return *this;
