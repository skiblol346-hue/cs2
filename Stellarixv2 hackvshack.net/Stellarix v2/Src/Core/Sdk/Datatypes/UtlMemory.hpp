...
public:
		...
		pMemory( nullptr ), nAllocationCount( nAllocationCount ), nGrowSize( nInitialGrowSize )
	{
		if ( nAllocationCount > 0 )
			pMemory = static_cast< T* >( GetInterfaceManager( )->GetMemAlloc( )->Alloc( nAllocationCount * sizeof( T ) ) );
	}
	...
	{
		return GetInterfaceManager( )->GetMemAlloc( )->Alloc( nSize );
	}

	void operator delete( void* pMemory )
	{
		GetInterfaceManager( )->GetMemAlloc( )->Free( pMemory );
	}
	...
	void Grow( const int nCount = 1 )
	{
		...
		if ( ( nAllocationCount + nCount ) > INT_MAX )
			return;
		...
		if ( pMemory != nullptr )
			pMemory = static_cast< T* >( GetInterfaceManager( )->GetMemAlloc( )->ReAlloc( pMemory, nAllocationCount * sizeof( T ) ) );
		else
			pMemory = static_cast< T* >( GetInterfaceManager( )->GetMemAlloc( )->Alloc( nAllocationCount * sizeof( T ) ) );
	}
	...
	void ConvertToGrowableMemory( int nInitialGrowSize )
	{
		...
		if ( nAllocationCount > 0 )
		{
			...
			T* pGrowableMemory = static_cast< T* >( GetInterfaceManager( )->GetMemAlloc( )->Alloc( nByteCount ) );
	...
	void Purge( )
	{
		...
		if ( pMemory != nullptr )
		{
			GetInterfaceManager( )->GetMemAlloc( )->Free( static_cast< void* >( pMemory ) );
			pMemory = nullptr;
		}
