...
#include <Core/Sdk/Datatypes/UtlHash.hpp>
#include <Core/Sdk/Datatypes/UtlVector.hpp>
...
public:
	...
	{
		return GetMemoryManager( )->CallVFunc<bool, 3U>( this, pOutSize, unkPtr );
	}
...
public:
	...
	const SchemaClassFieldData_t* pFields; // 0x0028
	char pad3[ 0x8 ]; // 0x0030
	const SchemaBaseClassInfoData_t* pBaseClasses; // 0x0038
	...
	bool InheritsFrom( SchemaClassInfoData_t* pClassInfo )
	{
		if ( pClassInfo == this && pClassInfo )
			return true;
		else if ( !pBaseClasses || !pClassInfo )
			return false;
...
public:
	...
	{
		return GetMemoryManager( )->CallVFunc<void, 2U>( this, pReturnClass, szClassName );
	}

	CSchemaType* FindSchemaTypeByName( const char* szName, std::uintptr_t* pSchema )
	{
		return GetMemoryManager( )->CallVFunc<CSchemaType*, 4U>( this, szName, pSchema );
	}

	CSchemaType* FindTypeDeclaredClass( const char* szName )
	{
		return GetMemoryManager( )->CallVFunc<CSchemaType*, 5U>( this, szName );
	}

	CSchemaType* FindTypeDeclaredEnum( const char* szName )
	{
		return GetMemoryManager( )->CallVFunc<CSchemaType*, 6U>( this, szName );
	}

	CSchemaClassBinding* FindRawClassBinding( const char* szName )
	{
		return GetMemoryManager( )->CallVFunc<CSchemaClassBinding*, 7U>( this, szName );
	}
...
public:
	...
	{
		return GetMemoryManager( )->CallVFunc<CSchemaSystemTypeScope*, 13U>( this, m_module_name, nullptr );
	}
